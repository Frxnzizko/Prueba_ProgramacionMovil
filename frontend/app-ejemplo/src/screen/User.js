...
const User = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://192.168.100.2:3000/nombres");
  ...
  const deleteUser = async (item) => {
    try {
      const response = await axios.delete(
        `http://192.168.100.2:3000/nombres/${item.id}`
      );
  ...
  const editUser = async () => {
    try {
      const response = await axios.put(
        `http://192.168.100.2:3000/nombres/${selectedUser.id}`,
        {
  ...
  const createUser = async () => {
    try {
      const response = await axios.post(
        "http://192.168.100.2:3000/nombres",
        {
          firstName: firstName,
          lastName: lastName,
        },
